...
GltfMeshBuilder::inspect (const tinygltf::Model & model) {
    ...
    for (size_t i = 0; i < model.meshes.size (); ++i) {
        ...
        for (size_t p = 0; p < mesh.primitives.size (); ++p) {
            ...
            std::cout << "\nFirst vertices:" << std::endl;


            size_t count = std::min < size_t >(accessor.count,
                                               10);
...
GltfMeshBuilder::build (const tinygltf::Model & model,
    ...
    std::cout << "Invoking build, meshName name : " << meshName << std::endl;
    ...
    if (model.nodes.empty ()) {
        std::cerr << "GLTF contains no nodes." << std::endl;
    ...
    //---------------------------------------------------------
    // Root node for imported model
    //---------------------------------------------------------

    Ogre::SceneNode * mainNode = parentNode->createChildSceneNode ();
    ...
    if (model.scenes.empty ()) {
        std::cerr << "GLTF contains no scenes." << std::endl;
    ...
    std::cout << "GLTF scene: " << scene.name << std::endl;
...
Ogre::SceneNode * GltfMeshBuilder::buildNode (const tinygltf::Model & model,
        int nodeIndex,
        Ogre::
    ...
    const
    tinygltf::Node &
    gltfNode = model.nodes[nodeIndex];
    ...
    if (nodeName.empty ()) {
        nodeName = meshName + "_node_" + std::to_string (nodeIndex);
    }
    ...
    if (gltfNode.translation.size () == 3) {
        node->setPosition (static_cast < Ogre::Real > (gltfNode.translation[0]),
    ...
    if (gltfNode.rotation.size () == 4) {
        Ogre::Quaternion q (static_cast < Ogre::Real > (gltfNode.rotation[3]),
    ...
    if (gltfNode.scale.size () == 3) {
        node->setScale (static_cast < Ogre::Real > (gltfNode.scale[0]),
    ...
    if (gltfNode.mesh >= 0) {
        buildMesh (model,
                   gltfNode.mesh, sceneManager, node, meshName, nodeIndex);
    }
...
void
GltfMeshBuilder::buildMesh (const tinygltf::Model & model,
                            int meshIndex,
                            Ogre::SceneManager *sceneManager,
                            Ogre::SceneNode *parentNode,
                            const std::string & meshName, int nodeIndex) {
    ...
    std::cout << "Build mesh " << meshIndex << ": " << mesh.name << std::endl;
    ...
    for (const tinygltf::Primitive & primitive:
        ...
        std::cout
                ...
                << " indices="
                << primitive.indices
                << " attributes=" << primitive.attributes.size () << std::endl;
        ...
        for (const auto & attr:
            ...
            std::cout
                    << "      "
                    << attr.first << " -> accessor " << attr.second << std::endl;
        }
        ...
        const size_t posStride =
            posView.byteStride != 0 ? posView.byteStride : 3 * sizeof (float);
        ...
        for (size_t i = 0; i < vertexCount; ++i) {
            const float *p =
                reinterpret_cast < const float *>(posData + i * posStride);


            positions[i] = Ogre::Vector3 (p[0], p[1], p[2]);
        }
        ...
        if (normalIt != primitive.attributes.end ()) {
            ...
            const size_t stride =
                view.byteStride != 0 ? view.byteStride : 3 * sizeof (float);
            ...
            for (size_t i = 0; i < accessor.count; ++i) {
                ...
                normals[i] = Ogre::Vector3 (n[0], n[1], n[2]);
            }
        }
        ...
        if (uvIt != primitive.attributes.end ()) {
            ...
            const size_t stride =
                view.byteStride != 0 ? view.byteStride : 2 * sizeof (float);
        ...
        for (size_t i = 0; i < indexAccessor.count; ++i) {
            ...
            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
                ...
                indices[i] =
                    reinterpret_cast < const unsigned char *>(indexData)[i];
            ...
            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
                ...
                indices[i] =
                    reinterpret_cast < const unsigned short *>(indexData)[i];
            ...
            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
                ...
                indices[i] =
                    reinterpret_cast < const unsigned int *>(indexData)[i];
        ...
        Ogre::HlmsUnlit * hlmsUnlit = mRenderer.getHlmsUnlit ();


        if (!hlmsUnlit) {
            std::cerr << "ERROR: HLMS Unlit is NULL!" << std::endl;
        ...
        if (!datablock) {
            std::cerr << "ERROR: GltfDefault datablock not found!" << std::endl;
        ...
        std::cout
                << "Using datablock: " << datablock->getNameStr () << std::endl;
        ...
        Ogre::ManualObject::ManualObjectSection * section = manual->end ();


        //-----------------------------------------------------
        // Explicitly assign HLMS datablock to section
        //-----------------------------------------------------

        if (section) {
            Ogre::HlmsDatablock * db =
                mRenderer.getHlmsUnlit ()->getDatablock ("GltfDefault");
            ...
            if (db) {
                ...
                std::cout
                        ...
                        << db << " name=" << db->getNameStr () << std::endl;
            } else {
                std::cerr
                        << "ERROR: GltfDefault datablock "
                        "not found when assigning section!" << std::endl;
            }
        } else {
            std::cerr
                    << "ERROR: ManualObject::end() "
                    "returned NULL section!" << std::endl;
        }
        ...
        std::cout << "  Created Ogre object: " << objectName << std::endl;
